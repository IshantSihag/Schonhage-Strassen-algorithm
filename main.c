...
BigInt Multiply_Bigint(const BigInt a, const BigInt b)
{
    unsigned int aLen = 1;
    while (aLen < a->len)
        aLen <<= 1;
    ...
    while (bLen < b->len)
        bLen <<= 1;
    unsigned int newLen = aLen;
    if (bLen>aLen)
        newLen = bLen;
    // newLen <<= 1;
    complex* a1;
    complex* b1;
    a1 = (complex*)malloc(2 * newLen * sizeof(complex));
    b1 = (complex*)malloc(2 * newLen * sizeof(complex));
    for(int i=0; i<2*newLen; i++)
    {
        a1[i].r = 0.0;
        a1[i].i = 0.0;
        b1[i].r = 0.0;
    ...
    poly_mul(a1, b1, 2*newLen);

    BigInt c = new_BigInt(2*newLen);
    set_zero(c);
    for(int i=0; i<c->len; i++)
    {
        if(a1[i].r<1e-10)
         continue;
        c->d[i] = (unsigned long long)a1[i].r;
    }
    remove_preceding_zeroes(c);
    return c;
}
